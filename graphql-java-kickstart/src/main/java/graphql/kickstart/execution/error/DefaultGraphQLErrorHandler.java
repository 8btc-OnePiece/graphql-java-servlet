...
public class DefaultGraphQLErrorHandler implements GraphQLErrorHandler {
    ...
    protected boolean isClientError(GraphQLError error) {
        if (error instanceof ExceptionWhileDataFetching) {
            ...
            return throwable instanceof GraphQLError || throwable.getCause() instanceof GraphQLError;
        }
        return true;
    }
    ...
    private GraphQLError replaceNonNullableFieldWasNullError(GraphQLError error) {
        if (error instanceof NonNullableFieldWasNullError) {
            ...
            Throwable throwable = ((ExceptionWhileDataFetching) error).getException();
            if (throwable instanceof CompletionException && throwable.getCause() instanceof GraphQLError) {
                return (GraphQLError) throwable.getCause();
            }
        }
        return error;
    }
